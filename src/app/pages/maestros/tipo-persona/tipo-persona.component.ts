...
@Component({
  ...
  styleUrls: ['./tipo-persona.component.sass']
})
export class TipoPersonaComponent implements OnInit {
  ...
  getTipoPersona() {
    this.apiService.getTipoPersona().subscribe((tipos: any[]) => {
      this.tipoPersonas = tipos
  ...
  deleteRow(row) {
    Swal.fire({
      ...
      text: "Seguro de eliminar Tipo Persona!",
      icon: 'warning',
      ...
      if (result.value) {
        this.spinner.show()
        this.apiService.deleteTipoPersona(row.id).subscribe({
          next: (v) => {
            this.spinner.hide();
            this.getTipoPersona()
            Swal.fire('Eliminado!', 'Tipo Persona Eliminada.', 'success');
          },
  ...
  onEditSave(form: FormGroup) {
    if (form.valid) {
      this.spinner.show()
      this.apiService.editTipoPersona(form.value).subscribe({
        next: (v) => {
          this.spinner.hide()
          this.getTipoPersona()
        },
  ...
  onAddRowSave(form: FormGroup) {
    ...
    this.spinner.show()
    this.apiService.addTipoPersona(this.data).subscribe({
      next: (v) => { this.spinner.hide(); this.getTipoPersona(); this.cerrarModal('add') },
      error: (v) => {
        this.spinner.hide();
      }
    });
  ...
  filterDatatable(event) {
    ...
    this.tipoPersonas = this.filteredData.filter((item) => {

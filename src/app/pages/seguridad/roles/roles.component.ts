...
export class RolesComponent implements OnInit {
  ...
  deleteRow(row) {
    Swal.fire({
      ...
      if (result.value) {
        this.spinner.show()
        this.apiService.deleteRol(row.id).subscribe({
          next: (v) => {
            this.spinner.hide();
            this.getData()
  ...
  onEditSave(form: FormGroup) {
    if (form.valid) {
      this.spinner.show()
      this.apiService.editRol(form.value).subscribe({
        next: (v) => {
          this.spinner.hide()
          this.getData()
        },
  ...
  onAddRowSave(form: FormGroup) {
    ...
    this.spinner.show()
    this.apiService.addRol(data).subscribe({

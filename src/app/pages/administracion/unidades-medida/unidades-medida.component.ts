...
@Component({
  ...
  styleUrls: ['./unidades-medida.component.sass']
})
 ...
 myControl = new FormControl();
  ...
  deleteRow(row) {
    Swal.fire({
      ...
      if (result.value) {
        this.spinner.show()
        this.apiService.deleteUnidadMedida(row.id).subscribe({
          next: (v) => {
            this.spinner.hide();
            this.getData()
  ...
  onEditSave(form: FormGroup) {
    if (form.valid) {
      this.spinner.show()
      this.apiService.editUnidadMedida(form.value).subscribe({
        next: (v) => {
          this.spinner.hide()
          this.getData();
  ...
  onAddRowSave(form: FormGroup) {
    ...
    this.spinner.show()
    this.apiService.addUnidadMedida(this.data).subscribe({
